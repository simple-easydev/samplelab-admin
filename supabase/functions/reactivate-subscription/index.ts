...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!subscription.cancel_at_period_end) {
      return jsonResponse(
        {
          message: "Subscription is not set to cancel; nothing to reactivate",
    ...
    if (!stripeSubscriptionId) {
      return jsonResponse(
        { error: "Subscription has no Stripe ID; cannot reactivate" },
        400
      );
    }
    ...
    // Clear cancel_at_period_end so the subscription continues
    ...
    return jsonResponse(
      {
        success: true,
        message:
          "Subscription reactivated. You will continue to be charged at the end of each billing period.",
        current_period_end: subscription.current_period_end,
      },
      200
    );
