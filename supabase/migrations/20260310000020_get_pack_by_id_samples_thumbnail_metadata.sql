-- Add thumbnail_url and metadata to samples array in get_pack_by_id response.
-- Run after 20260310000019_get_pack_by_id has been applied.
...
BEGIN
  ...
  -- Samples now include thumbnail_url and metadata
...
COMMENT ON FUNCTION public.get_pack_by_id(uuid) IS
  'Returns one JSONB object for a pack: id, name, description, cover_url, creator_id, creator_name, category_id, category_name, genres[], tags[], is_premium, samples_count, download_count, status, created_at, samples[] (with thumbnail_url, metadata), similar_packs[]. Returns null if pack not found. For pack detail page.';
