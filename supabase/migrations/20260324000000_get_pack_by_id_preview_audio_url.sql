-- Add preview_audio_url to samples[] in get_pack_by_id (full audio stays in audio_url).
...
COMMENT ON FUNCTION public.get_pack_by_id(uuid) IS
  'Returns one JSONB object for a pack: id, name, description, cover_url, creator_id, creator_name, category_id, category_name, genres[], tags[], is_premium, samples_count, download_count, status, created_at, samples[] (audio_url, preview_audio_url, thumbnail_url, metadata), similar_packs[]. Returns null if pack not found. For pack detail page.';
