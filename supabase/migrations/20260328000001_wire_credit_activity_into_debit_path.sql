-- Wire credit ledger into existing debit path.
-- Updates request_sample_download_prepare to write public.credit_activity
-- in the same transaction as the customer balance update.
...
COMMENT ON FUNCTION public.request_sample_download_prepare(uuid, uuid, uuid) IS
  'Debit credits, write credit_activity ledger row, and record download; returns storage info for Edge signing.';
