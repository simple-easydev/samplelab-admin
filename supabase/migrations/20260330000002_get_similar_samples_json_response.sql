-- Return { seed_sample: { id, name }, similarities: [...] } as jsonb.
-- Return type changed from SETOF record → jsonb; must DROP first.
-- Version: after get_similar_packs_by_liked_pack (no dependency; avoids duplicate 20260329000003).
...
COMMENT ON FUNCTION public.get_similar_samples_by_downloaded_sample(integer) IS
  'Returns { seed_sample: { id, name }, similarities: [...] }. Seed from last credit_activity download_charge.';
