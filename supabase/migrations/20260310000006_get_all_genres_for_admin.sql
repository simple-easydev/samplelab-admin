...
-- Frontend: supabase.rpc('get_all_genres_for_admin')
-- RLS: runs as invoker; admins see all genres.

CREATE OR REPLACE FUNCTION public.get_all_genres_for_admin()
RETURNS TABLE (
  id uuid,
  name text,
  description text,
...
COMMENT ON FUNCTION public.get_all_genres_for_admin() IS
  'Returns all genres with packs_count and samples_count. For admin library Genres tab.';

GRANT EXECUTE ON FUNCTION public.get_all_genres_for_admin() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_all_genres_for_admin() TO service_role;
