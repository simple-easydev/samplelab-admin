-- Fix p_instrument: match metadata key case-insensitively (instrument / Instrument) and
-- match value as substring (case-insensitive) so "guitar" matches "Guitar" and "Acoustic Guitar".
        ...
        OR (
          ...
          AND EXISTS (
            ...
            WHERE lower(kv.k) = 'instrument'
              ...
              AND lower(trim(kv.v)) LIKE '%' || lower(trim(p_instrument)) || '%'
          )
        )
      )
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. p_instrument matches metadata key "instrument" (any casing) and value substring (case-insensitive).';
