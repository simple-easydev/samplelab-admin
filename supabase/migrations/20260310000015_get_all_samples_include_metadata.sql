-- Include metadata (waveform bars, duration_seconds) in get_all_samples response
...
COMMENT ON FUNCTION public.get_all_samples() IS
  'Returns one row per sample with pack, creator, genre, stem count, and metadata (bars, duration_seconds). For admin library Samples tab.';
