-- Add audio_url to get_all_samples response (drop + recreate to change return type)
...
COMMENT ON FUNCTION public.get_all_samples() IS
  'Returns one row per sample with pack, creator, genre, stem count, audio_url, and metadata. For admin library Samples tab.';
