-- Remove legacy no-argument get_all_samples() if present (PostgREST could call it and ignore filters).
-- p_instrument: single COALESCE(column, metadata->instrument, case-insensitive jsonb key) then ILIKE match.
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Samples list with filters. p_instrument uses COALESCE(samples.instrument, metadata.instrument). Drops legacy no-arg overload.';
