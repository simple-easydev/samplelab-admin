-- Extend get_all_samples with optional filters and sort. Backward compatible: no args = same as before.
-- New params (all optional with defaults). Returns same row shape + total_count.
...
$$
COMMENT ON FUNCTION public.get_all_samples(text, text, text[], text[], text, text, text, text[], text, integer, integer, integer, integer, integer) IS
  'Returns samples with optional filters and sort. Params: p_search (sample/pack/creator name ILIKE), p_sort (newest|oldest|popular|name-az|name-za), p_genres (genre names via pack), p_keywords (pack tags overlap), p_instrument (all; reserved), p_type (all|loop|one-shot), p_stems (all|yes|no), p_keys (key values), p_key_quality (all; reserved), p_bpm_min/max/exact (exact overrides range), p_limit, p_offset. total_count = total matching rows before limit/offset.'
GRANT EXECUTE ON FUNCTION public.get_all_samples(text, text, text[], text[], text, text, text, text[], text, integer, integer, integer, integer, integer) TO authenticated
GRANT EXECUTE ON FUNCTION public.get_all_samples(text, text, text[], text[], text, text, text, text[], text, integer, integer, integer, integer, integer) TO service_role