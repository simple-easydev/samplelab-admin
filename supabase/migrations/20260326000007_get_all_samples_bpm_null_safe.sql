-- BPM: p_bpm_min 0 or NULL = no lower bound; NULL bpm passes when min is open (so default 0–240 no longer drops unknown BPM).
-- Instrument / other behavior unchanged from 20260326000006.
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. p_instrument: column or metadata.instrument. BPM: min<=0 or null = no lower bound; null bpm allowed when min is open.';
