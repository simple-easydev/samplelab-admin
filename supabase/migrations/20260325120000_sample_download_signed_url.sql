-- Atomic credit check + debit for full-sample downloads; Edge Function creates signed URLs.
-- See Edge Function: request-sample-download

-- Audit / idempotency: one row per successful debit (or idempotent replay uses existing row)
CREATE TABLE IF NOT EXISTS public.sample_credit_downloads (
...
CREATE UNIQUE INDEX IF NOT EXISTS sample_credit_downloads_user_idempotency_uidx
  ON public.sample_credit_downloads (user_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS sample_credit_downloads_user_created_idx
  ON public.sample_credit_downloads (user_id, created_at DESC);

ALTER TABLE public.sample_credit_downloads ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE public.sample_credit_downloads IS
  'Records sample full-audio downloads after credits are charged; used for audit and idempotent retries.';

-- Full file path / URL is stored on public.samples.audio_url (see samples table).
INSERT INTO storage.buckets (id, name, public)
VALUES ('private-audios', 'private-audios', false)
ON CONFLICT (id) DO NOTHING;

-- Full audio bucket: private; access via signed URLs from Edge after credit check.
UPDATE storage.buckets
SET public = false
WHERE id = 'private-audios';
...
BEGIN
  ...
  INTO v_name, v_audio_url, v_cost, v_sample_status, v_pack_status, v_is_premium
  FROM public.samples s
...
COMMENT ON FUNCTION public.request_sample_download_prepare(uuid, uuid, uuid) IS
  'Debit credits and record download; returns storage bucket + audio_url for Edge to sign. Service role only.';
