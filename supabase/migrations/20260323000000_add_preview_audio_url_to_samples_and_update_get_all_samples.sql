-- Add preview_audio_url to samples and expose it in get_all_samples.
-- We keep `audio_url` in the table for full-audio downloads (credit-locked),
-- but the Samples RPC returns only the preview URL to power lightweight playback.

ALTER TABLE public.samples
  ADD COLUMN IF NOT EXISTS preview_audio_url text;

-- Backfill previews for existing rows (until preview audio is generated).
UPDATE public.samples
SET preview_audio_url = audio_url
WHERE preview_audio_url IS NULL;

-- Update admin library RPC shape: preview_audio_url instead of audio_url.
