-- Do not charge credits for re-downloading a sample that the user already downloaded,
-- while still enforcing subscription + access rules.
--
-- Option A: re-download is free, but still requires active subscription for premium packs.

CREATE INDEX IF NOT EXISTS sample_credit_downloads_user_sample_idx
  ON public.sample_credit_downloads (user_id, sample_id);
...
BEGIN
  ...
  -- If user already downloaded this sample before, do not charge again.
  -- Subscription and access checks above still apply (Option A).
  IF EXISTS (
    SELECT 1
    FROM public.sample_credit_downloads scd
    WHERE scd.user_id = p_user_id
      AND scd.sample_id = p_sample_id
    LIMIT 1
  ) THEN
    RETURN jsonb_build_object(
      'ok', true,
