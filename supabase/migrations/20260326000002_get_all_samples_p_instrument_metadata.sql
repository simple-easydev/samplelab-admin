-- Wire p_instrument to samples.metadata->>'instrument' (e.g. "Guitar"); 'all' / NULL skips filter.
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          COALESCE(s.metadata->>'instrument', '') <> ''
          AND lower(trim(s.metadata->>'instrument')) = lower(trim(p_instrument))
        )
      )
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. p_instrument filters by metadata.instrument (case-insensitive). Key filters: p_keys / p_key_quality as documented.';
