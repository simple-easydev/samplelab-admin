-- Backfill samples.instrument from metadata when still null (e.g. "Drums" only in JSON).
-- p_instrument: match on column OR, if column empty, on metadata.instrument (case-insensitive ILIKE, bidirectional).
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          AND (
            (
              NULLIF(trim(s.instrument), '') IS NOT NULL
              AND (
                trim(s.instrument) ILIKE '%' || trim(p_instrument) || '%'
                OR trim(p_instrument) ILIKE '%' || trim(s.instrument) || '%'
              )
            )
            OR (
              NULLIF(trim(s.instrument), '') IS NULL
              AND EXISTS (
                SELECT 1
                ...
                WHERE lower(kv.k) = 'instrument'
                  AND NULLIF(trim(kv.v), '') IS NOT NULL
                  AND (
                    trim(kv.v) ILIKE '%' || trim(p_instrument) || '%'
                    OR trim(p_instrument) ILIKE '%' || trim(kv.v) || '%'
                  )
              )
            )
          )
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. p_instrument matches samples.instrument or metadata.instrument (ILIKE, bidirectional substring).';
