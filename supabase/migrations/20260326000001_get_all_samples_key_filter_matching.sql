-- Align get_all_samples key filters with stored key text (e.g. "B Major", "F# Minor"):
-- - p_keys: case-insensitive; each value matches either the full key or the pitch-class prefix
--   (e.g. "F#" or "f#" matches "F# Minor").
-- - p_key_quality: major | minor narrow by suffix; all / NULL / unknown = no quality filter.
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. Key filters: p_keys matches full key text (case-insensitive) or pitch-class prefix (e.g. F# matches F# Minor); p_key_quality major|minor filters by suffix; all/NULL/unknown skips quality filter.';
