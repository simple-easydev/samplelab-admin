-- p_instrument: match when filter is contained in instrument OR instrument is contained in filter
-- (e.g. "drums" matches "Drum", "Acoustic Drums"; "guitar" matches "Guitar").
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          trim(p_instrument) <> ''
          AND NULLIF(trim(s.instrument), '') IS NOT NULL
          AND (
            lower(trim(s.instrument)) LIKE '%' || lower(trim(p_instrument)) || '%'
            OR lower(trim(p_instrument)) LIKE '%' || lower(trim(s.instrument)) || '%'
          )
        )
      )
...
COMMENT ON FUNCTION public.get_all_samples(
  ...
  'Returns samples with optional filters and sort. p_instrument matches samples.instrument (bidirectional case-insensitive substring: drum vs drums).';
