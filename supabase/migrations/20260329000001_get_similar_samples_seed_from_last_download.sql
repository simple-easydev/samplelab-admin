-- Update RPC: Similar samples for the caller's last downloaded sample.
-- Similarity v1: other samples in the same pack (excluding the seed sample).
-- Seed selection: the most recent credit ledger row with activity_type = 'download_charge'.
...
BEGIN
  ...
  -- Seed = last downloaded sample (from credit ledger).
  SELECT ca.sample_id
...
COMMENT ON FUNCTION public.get_similar_samples_by_downloaded_sample(integer) IS
  'Returns similar samples for the caller’s last downloaded seed sample. v1: same pack; seed from credit_activity(download_charge).';
