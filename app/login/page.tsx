...
function LoginContent() {
  ...
  // Redirect to setup page when invite token is present (must run in useEffect, not during render)
  useEffect(() => {
    if (inviteToken) {
      setRedirecting(true);
      router.replace(`/auth/setup?token=${inviteToken}`);
    }
  }, [inviteToken, router]);

  // Show nothing while redirecting to avoid setState-during-render
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Note:</strong> This is an invite-only admin system. Contact your administrator
            if you need access.
          </p>
        </div>
      </div>
